...
def display_menu_expanded():
    ...
    for daily in weekly_menu:
        # 每一天一个大卡片
        st.markdown(f"### {daily.day} {daily.date}")

        # 早餐
        if daily.breakfast:
            st.markdown("#### 🍳 早餐")
            for dish in daily.breakfast:
                with st.container(border=True):
                    col1, col2 = st.columns([1, 3])
                    with col1:
                        st.markdown(f"**{get_dish_category_label(dish.category)}**")
                    with col2:
